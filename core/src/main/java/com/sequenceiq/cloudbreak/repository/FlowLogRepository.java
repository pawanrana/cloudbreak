...
public interface FlowLogRepository extends CrudRepository<FlowLog, Long> {
    ...
    @Query("SELECT DISTINCT fl.flowId, fl.stackId FROM FlowLog fl "
            + "WHERE (fl.finalized IS NULL OR fl.finalized = false) AND fl.cloudbreakNodeId = :cloudbreakNodeId ")
    List<Object[]> findAllRunningFlowIdsByCloudbreakId(@Param("cloudbreakNodeId") String cloudbreakNodeId);
